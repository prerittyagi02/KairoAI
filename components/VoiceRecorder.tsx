...
import { Loader2, Mic, MicOff, Play, Pause, Volume2 } from "lucide-react";
...
export default function VoiceRecorder({ onRecordingComplete, isProcessing }: Props) {
  ...
  const buttonClass = useMemo(
    () =>
      cn(
        ...
        isRecording ? "bg-rose-500 text-white hover:bg-rose-600" : "bg-teal-600 text-white hover:bg-teal-700"
      ),
    [isRecording]
  );
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Voice Symptom Recorder</h3>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
            Speak your symptoms to get a quick analysis. Press the microphone to start recording.
          </p>
        </div>
        <div className="flex gap-2">
          <Button onClick={isRecording ? stopRecording : startRecording} className={buttonClass} disabled={!canRecord || !!isProcessing}>
